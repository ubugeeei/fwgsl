//! Module resolution for multi-file fwgsl projects.
//!
...
///
/// - `root_path`: the path to the root .fwgsl file
...
impl<'a> Resolver<'a> {
    ...
    fn find_module_file(&self, module_path: &str) -> Option<PathBuf> {
        let relative = module_path.replace('.', "/") + ".fwgsl";
...
mod tests {
    ...
    fn single_file_no_imports() {
        ...
        let root = PathBuf::from("/project/Main.fwgsl");
...
"#;
        ...
        reader.add(PathBuf::from("/project/Utils.fwgsl"), utils_source);
...
"#;
        ...
        let mut reader = MemoryReader::new();
        reader.add(PathBuf::from("/p/A.fwgsl"), a_source);
        reader.add(PathBuf::from("/p/B.fwgsl"), b_source);

        let root = PathBuf::from("/p/Main.fwgsl");
...
"#;
        ...
        let root = PathBuf::from("/p/A.fwgsl");
        let result = resolve_modules(&root, root_program, &[PathBuf::from("/p")], &reader);
        match result {
            Err(errors) => {
                assert!(errors
...
"#;
        ...
        let mut reader = MemoryReader::new();
        reader.add(PathBuf::from("/p/Math/Fp64.fwgsl"), fp64_source);
    ...
    fn diamond_dependency() {
        ...
        reader.add(PathBuf::from("/p/A.fwgsl"), a_source);
        reader.add(PathBuf::from("/p/B.fwgsl"), b_source);
        reader.add(PathBuf::from("/p/C.fwgsl"), c_source);
