...
/// Module names are mapped to paths: `Math.Vec` → `Math/Vec.fwgsl`.
///
/// Example input:
/// ```text
/// --- module Math.Vec ---
...
/// Convert a dotted module name to a file path: `Math.Vec` → `Math/Vec.fwgsl`.
fn module_name_to_path(name: &str) -> PathBuf {
    let relative = name.replace('.', "/") + ".fwgsl";
...
mod tests {
    ...
    fn virtual_fs_basic() {
        let mut fs = VirtualFs::new();
        fs.add("Main.fwgsl", "f x = x");
        fs.add("Utils.fwgsl", "g x = x + 1");
    ...
    fn virtual_fs_list_dir() {
        let mut fs = VirtualFs::new();
        fs.add("Math/Vec.fwgsl", "dot2 a b = 0.0");
        fs.add("Math/Matrix.fwgsl", "identity = 1.0");
        fs.add("Main.fwgsl", "f x = x");
...
"#;
        ...
        assert!(fs.file_exists(Path::new("Math/Vec.fwgsl")));
        assert!(fs.file_exists(Path::new("Main.fwgsl")));

        let vec_src = fs.read_source(Path::new("Math/Vec.fwgsl")).unwrap();
        assert!(vec_src.contains("dot2"));

        let main_src = fs.read_source(Path::new("Main.fwgsl")).unwrap();
