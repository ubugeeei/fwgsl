...
pub fn format(source: &str) -> String {
    shadml_formatter::format_default(source)
}
...
pub fn editor_completions(source: &str, line: u32, column: u32) -> String {
    let items = shadml_ide::build_completions(source, editor_position(line, column))
        .into_iter()
...
pub fn editor_hover(source: &str, line: u32, column: u32) -> String {
    let hover =
        shadml_ide::build_hover(source, editor_position(line, column)).map(editor_hover_output);
...
pub fn editor_definition(source: &str, line: u32, column: u32) -> String {
    let uri = editor_uri();
    let locations =
        match shadml_ide::build_goto_definition(&uri, source, editor_position(line, column)) {
...
pub fn editor_references(
    ...
    let uri = editor_uri();
    let locations = shadml_ide::build_references(
        &uri,
        source,
...
fn convert_diagnostic(diag: &shadml_diagnostics::Diagnostic, source: &str) -> DiagnosticOutput {
...
fn editor_uri() -> Url {
    Url::parse("inmemory://shadml/playground.shadml").expect("static playground URI")
}
